 ...
 * @global string $ef3_import_results
 */
function ef3_widgets_process_import_file($part)
{
	...
	$wie_import_results = ef3_widgets_import_data($data);
 ...
 */
function ef3_widgets_import_data($data)
{
	...
	// Hook before import
	do_action('ef3_before_import');
	$data = apply_filters('ef3_import_data', $data);
	...
	foreach ($data['sidebars'] as $sidebar_id => $widgets) {
		...
		foreach ($widgets as $widget_instance_id => $widget) {
			...
			// Ideally the newer ef3_widget_settings_array below will be used instead of this
			$widget = apply_filters('ef3_widget_settings', $widget); // object
			...
			// This is preferred over the older ef3_widget_settings filter above
			// Do before identical check because changes may make it identical to end result (such as URL replacements)
			$widget = apply_filters('ef3_widget_settings_array', $widget);
			...
			if (!$fail) {
				...
				do_action('ef3_after_widget_import', $after_widget_import);
	...
	// Hook after import
	do_action('ef3_after_import');

	// Return results
	return apply_filters('ef3_import_results', $results);
 ...
 */
function ef3_available_widgets() {
	...
	return apply_filters( 'ef3_available_widgets', $available_widgets );
