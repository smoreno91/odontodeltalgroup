...
class WP_Import extends WP_Importer {
	 ...
	 */
	function import( $file, $attachment ) {
		...
		$this->process_tags();
		$this->process_terms();
		$this->process_posts($attachment);
		wp_suspend_cache_invalidation( false );

		// update incorrect/missing information in the DB
		$this->backfill_parents();
		$this->backfill_attachment_urls();
		$this->remap_featured_images($attachment);
...
<?php
	 ...
	 */
	function process_posts($attachment) {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				$postdata = array(
					...
					'post_date_gmt' => $post['post_date_gmt'], 'post_content' => ef3_replace_content($post['post_content'], $attachment),
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					$comment_post_ID = $post_id = !$attachment ? $this->process_attachment( $postdata, $remote_url ) : $attachment ;
				} else {
	...
	function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			foreach ($item['postmeta'] as $_meta) {
				...
				if (!isset($_meta['key']) || !isset($_meta['value']) || in_array($_meta['key'], $_nav_meta_wp_key))
					continue;

				update_post_meta($id, $_meta['key'], $_meta['value']);
			};
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if(file_exists($_urlxc)){
			$upload = array(
				'file' => $_urlxc,
				'url'  => SITE_URI . '/wp-content' . $_urlxz[1]
			);
		} else {
	 ...
	 */
	function remap_featured_images($attachment) {
		// cycle through posts that have a featured image
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->processed_posts[$value] ) ) {
				$new_id = !$attachment ? $this->processed_posts[$value] : $attachment;
	 ...
	 * @return int 60
	 */
	function bump_request_timeout($_time_out = 60) {
		return 60;
	}
