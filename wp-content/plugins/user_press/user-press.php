...
if (! class_exists('UserPress')) {
    ...
    final class UserPress
    {
        ...
        private function setup_actions()
        {
            ...
            //add_action('add_routes', array($this, 'add_site_routes'));
        }

        function custom_query_vars( $vars ) {
            $vars[] = 'user-press/login_twitter';
            return $vars;
        }

        function custom_requests ( $wp ) {
            $valid_actions = array('user-press/login_twitter');

            if( !empty($wp->query_vars['user-press/login_twitter']) &&
                in_array('user-press/login_twitter', $valid_actions)
            ) {
                add_action();
            }
        }
        ...
        {
            require_once userpress()->plugin_dir . 'inc/routes.php';
        ...
        /* social scripts */
            wp_enqueue_script('all.js', 'https://connect.facebook.net/en_US/all.js', array('jquery'), '1.0.0' , true);
            /* Facebook */
            ...
            /* Twitter */
            wp_register_script( 'twitter.login.js', userpress()->acess_url . 'js/twitter.login.js', array('jquery'), '1.0.0' , true);
            wp_localize_script( 'twitter.login.js', 'userpress', array( 'ajax' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce(UserPress::NONCE), 'app_id' => $app_id['google'] ) );
            wp_enqueue_script( 'twitter.login.js');
            /* Instagram */
            /* Linked */
            /* Social icons */
