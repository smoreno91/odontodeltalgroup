...
if (! class_exists('UserPress_Ajax')) {
    ...
    class UserPress_Ajax
    {
        ...
        function __construct() {
            ...
            /* TWITTER AJAX LOGIN */
            add_action('wp_ajax_nopriv_twitter_ajax_login', array(
                $this,
                'twitter_ajax_login_callback'
            ));
            /* action register */
        ...
        function form_ajax_login_callback()
        {
            ...
            /* login. */
            if(self::login_action($login_data['user'], $login_data['pass'], $login_data['rememberme']))
                die(json_encode((object)array('error'=> false)));
        ...
        function login_action($user, $pass, $rememberme = false)
        {
            ...
            /* get user by name. */
            $user_result = get_user_by('slug', $user);

            /* set login. */
            wp_set_auth_cookie( $user_result->data->ID, $rememberme, false );
        ...
        function twitter_ajax_login_callback() {

            $args = array(
                'Authorization' => 'OAuth oauth_consumer_key="1JUnmzVdJPHwMSESSqAzeI2H6", oauth_nonce="72e147727f89c18632b69736341fc314", oauth_signature="HjHeBmdeqWrk5wkRu4qdt14ooAo%3D", oauth_signature_method="HMAC-SHA1", oauth_timestamp="1475741168", oauth_token="839676980-SvH9uk3ZKXlH0IkwofVaiS22p4OtwwCKcpFGvg0h", oauth_version="1.0"'
            );
            $resp = wp_remote_post( 'https://api.twitter.com/oauth/request_token', $args);
            var_dump($resp);
            exit();
        }
    }
