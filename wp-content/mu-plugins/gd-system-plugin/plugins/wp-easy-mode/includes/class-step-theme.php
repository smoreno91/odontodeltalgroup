...
final class Step_Theme extends Step {
	...
	private function header_image_list( $offset = 0 ) {
		...
		foreach ( $images as $key => $image ) :
			...
			$image_url          = sprintf( '%s/%s/:/rs=w:2400/qt=q:50', untrailingslashit( $image_api_base_url ), wp_basename( $image->url ) );

			?>
			<li <?php echo ( 0 === $offset && 0 === $key ) ? 'class="selected"' : '' ?> >
				<a href="#"
				   data-image-preview-url="<?php echo esc_url( $image->url ); ?>"
