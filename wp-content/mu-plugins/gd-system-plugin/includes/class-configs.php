...
final class Configs {
	...
	public function get( $config ) {
		 ...
		 * @since 3.0.1
		 */
		if ( in_array( $config, [ 'd3.categories', 'd3.v4.categories' ], true ) && ! isset( $this->data[ $config ] ) ) {
	...
	public function d3_categories( $config ) {
		...
		switch ( $config ) {
			...
			case 'd3.v4.categories':
				$this->data['d3.v4.categories'] = $this->parse_json(
					$this->find_readable_path(
						[
							'/var/chroot/web/conf/d3-v4-categories.json',
							WPMU_PLUGIN_DIR . '/bin/d3-categories.json',
						]
					)
				);
