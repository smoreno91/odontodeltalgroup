...
final class Temp_Domain {
	...
	public function __construct( API_Interface $api ) {
		...
		if ( ! Plugin::is_staging_site() ) {
			...
			$message = sprintf(
				...
				__( '<strong>Note:</strong> You\'re using the temporary domain <strong>%1$ss</strong>. <a href="%2$s" target="_blank">Change domain</a>', 'gd-system-plugin' ),
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( Plugin::is_staging_site() ) {
			...
			$url = Plugin::account_settings_url( 'cname_link' );
